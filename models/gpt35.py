...
def eval_gpt35(df, prompt_type=0, load_cache=False):
    ...
    pd.DataFrame(results, columns=['prompt', 'response_str', 'ID', 'model']).to_csv(f"cache/gpt35_{prompt_type}.csv")
    return results
    
df = pd.read_csv('MPLU_text.csv')    

res = eval_gpt35(df, load_cache=True)